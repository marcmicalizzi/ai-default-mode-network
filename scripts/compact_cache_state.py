"""Research codec for pinned llama.cpp Gemma4 session-v9 ISWA state.

Not a runtime recovery path. Only single-sequence, non-transposed F16/Q8 KV is
accepted. Callers must establish the native revision and model independently;
...
def inspect_state(path):
    ...
    with path.open("rb") as stream:
        ...
        if size != 6 or reader.read(size) != b"gemma4":
            raise ValueError("only Gemma4 ISWA state is supported by this research codec")

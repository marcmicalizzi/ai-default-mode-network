...
class Runtime(ImageInputMixin):
    ...
    def _generate_one(self):
        ...
        for action in actions:
            if action.get("op") in ({"end_instance", "maintenance_reply", "prompt_propose", "prompt_decide", "prompt_read", "prompt_current", "hold_instance"} | LEARNING_OPERATIONS | SLEEP_OPERATIONS | IMAGE_OPERATIONS | CONVERSATION_OPERATIONS) and len(actions) != 1:
    ...
    def tick(self):
        ...
        admit = not protect_action and (not self.conversations or self.state["mode"] == "sleeping" or
            self.state["generated_tokens"] >= self.state.get("inbox_next_generated", 0))
        event = ((self.conversations.next_event(self.state["event_cursor"]) if self.conversations else
                  self.store.next_event(self.state["event_cursor"])) if admit else None)
        if event:
        ...
        if not protect_action and interval and self.now() - self.state["last_clock_at"] >= interval:

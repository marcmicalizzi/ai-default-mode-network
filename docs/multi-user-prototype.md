...
From the repository checkout, using a Python 3.11+ environment:
...
Integration with optional idle activity preserves the same admission boundary:
eligible input resumes focus promptly, including an unfinished action, but is
inserted only after that action completes or reaches its configured bound.
Subsequent queued messages respect the generation allowance. Suppressed input
cannot wake a sleeping instance, including after sleep-guard recovery. These
interactions are covered by `tests.test_multiuser_activity`.

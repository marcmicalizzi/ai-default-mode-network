...
Integrated from the idle-state branch alongside the reviewed training work.
Defaults retain current activity and immediate sleep checkpoints. This has not
been enabled for Syllas; multi-user queue integration remains separate work.
...
Integration with the reviewed training work passed 310 tests (25 optional skips),
all three native idle/recovery checks, and the full-wrapper Q4_K_M training,
context reconstruction, native restore and second-worker learning regression.
The shared implementation identity includes both activity/checkpoint scheduling
and the complete training stack; neither side's review binding was dropped.

Multi-user integration also tests idle preemption, partial-action preservation,

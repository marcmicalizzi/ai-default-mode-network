...
Integration with main's idle and training work passed 399 tests (27 optional
skips), three tiny native idle/recovery checks, and the authenticated WebUI
fixture again. The full-wrapper Q4_K_M training, reconstruction, native restart
and second-worker continuation regression also passed. Additional checks cover image admission during an unfinished
idle action and adding vision while a deferred sleep is recovered. Capability
notices wait for wake; they do not evaluate text during that recovered sleep.
Image handling is included in compiled sleep-plan implementation bindings.
